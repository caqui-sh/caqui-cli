...
* **Schema & Type Integrity**:
  ...
  - Each object must contain `existingTestStep` if and only if the scenario modifies an established boundary contract. For all other scenarios, this field is prohibited and must be omitted.
...
* **Test Step Reference Verification**:
  - The unique 6-character identifier specified in `existingTestStep` must resolve to an active test case or step within the preexisting E2E test files under `/meta/tests/`.
